...
\label{cayleyZ2}
\end{figure}
これは，群の元を頂点とし，生成系の元で辺を結ぶことで得られるグラフであり，(生成系に依存はするものの) 生成系込みでの群の構造を可視化することに秀でている．特に自由群に関しては相性が良く，そのCayleyグラフは tree になることが知られている．またこのことから，自由群の部分群が自由群になるスマートな証明も与えられる．

Cayleyグラフ から自然と得られるような距離を群に導入することで，群を距離空間として捉えることができる．そして他の距離空間への群作用を見ることで，群作用をしている距離空間の構造と，もともとの群の構造の類似性を考察することができる．例えば，上の図 \ref{cayleyZ2} では，$\mathbb{Z}^2$ の Cayleyグラフは $\mathbb{R}^2$ のような広がりを持っていることが視覚的に理解できる．そのため，$\mathbb{Z}^2$ という群の構造は，$\mathbb{R}^2$ という空間と同じような構造を持っていることが分かる．この際，Cayleyグラフの構造は生成系に依存してしまうため，群に導入した距離関数も生成系に依存してしまう．そこで，一定誤差のズレを許容する“大雑把に粗く見積もる視点”を導入することで，この問題を解決できるようになる．
...
群に組み込む距離関数を word metric にした場合，その定義からこの距離空間は生成系に大きく依存するため，距離空間と群が同じ構造を持つかどうかの議論が難しくなる．しかし，擬等長という“大雑把な視点”で見ると，生成系に依らずに議論することが可能である．
...
\appendix
\section{付録}
ここでは幾何学的群論の分野にて基本的な概念となる Cayleyグラフ や，その周辺の事項について述べる．なおここで述べるグラフとは，・・・・
...
\begin{dfn}
  $M$ を位相多様体とする．各点 $p\in M$ における接空間 $T_p(M)$ 上の内積 $g_p$ があり，それが $M$ 全体で $C^\infty$ 級のとき，$(M,\{g_p\}_{p\in M})$ を \textbf{リーマン多様体} という．また，$p\in M$ における接空間 $T_p(M)$ 上のベクトル $u\in T_p(M)$ に関して，
  \begin{equation*}
    \|u\|:=\sqrt{g_p(u,u)}
  \end{equation*}
