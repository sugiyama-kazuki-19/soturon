...
\section{準備}
\subsection{Quasi-Isometry of Meric Spaces}
$(X,d_X)$ を距離空間とし，省略して $X$ と表記する．

\begin{dfn}\label{isom}
写像 $f\colon X\rightarrow Y$ が \textbf{等長埋め込み (isometry embedding)} であるとは，任意の $x,x^\prime\in X$ に関して，$d_X(x,x^\prime)=d_Y(f(x),f(x^\prime))$ が成り立つことをいう．また，$f$ が \textbf{等長写像 (isometry)} であるとは，以下の二条件を満たすときをいう．
\begin{enumerate}
  \item $f$ が等長埋め込みである．
