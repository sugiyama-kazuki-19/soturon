...
Cayleyグラフとは，群の元を頂点とし，生成系の元で辺を結ぶことで得られるグラフであり，生成系によってグラフの形は変わるものの，生成系込みでの群の構造を可視化することに長けている．特に自由群に関しては相性が良く，そのCayleyグラフは tree になることが知られている．逆に，適切な条件下であれば，群と生成系に関するCayleyグラフが tree のとき群は自由群であることも知られている．また，Cayleyグラフは群が作用する空間の代表例と言える．実際，群は自身のCayleyグラフの頂点集合に対して，左からの演算で群作用をなす．この群作用は，サイクルなどのグラフの構造を保つため，Cayleyグラフの構造を群作用を通して理解することもできる．また，自由群が作用する場合，群作用が自由になることから，自由群の部分群が自由群になるスマートな証明も与えらえる．幾何学的群論ではしばしば群作用に着目することが多い．
...
本節では，\ref{intro} 章でも記した，“よい”距離空間について述べる．一般に距離空間では，二点が与えられたときに二点間の距離を計算できるが，その距離の長さ通りのパスを距離空間上に作ることができるとは限らない．そのようなことが可能であるような距離空間を測地的といい，その条件をさらに緩めたものを擬測地的という．擬測地的な距離空間では，二点間のパスの長さが，二点間の距離の定数倍と定数項で抑えられるような条件さえ満たせればよい．
\begin{dfn}
...
$G$ を群とし，$(c,b)$-測地的な距離空間 $(X,d_X)$ 上に等長な群作用があるとする．もしある部分集合 $B\subset X$ が存在し，
   \begin{enumerate}
...
\begin{dfn}\label{homotopy}
  $X$ を位相空間とする．$I\coloneqq[0,1]$ として，連続写像 $f\colon I\rightarrow X$ を \textbf{パス (path)} といい，$f(0)=f(1)$ のときを特に \textbf{ループ (loop)} という．また，パスの族 $f_t\colon I\rightarrow X\;(0\leq t\leq 1)$ が \textbf{ホモトピー (homotopy)} であるとは，以下の二条件：
  \begin{enumerate}
    \item $f_t(0)=x_0$，$f_t(1)=x_1$ が $t\in I$ に依らない．
    \item $F(s,t)\coloneqq f_t(s)$ が，$s,t$ に関して連続．
  \end{enumerate}
  が成り立つときをいう．このとき，パス $f_0$ と $f_1$ を \textbf{ホモトピック (homotopic)} であるといい，$f_0\simeq f_1$ と表記する．
\end{dfn}
...
\begin{dfn}
  ...
  \end{enumerate}
  が成り立つとき，$\widetilde{X}$ と $p$ の組 $(\widetilde{X},p)$ を $X$ の \textbf{被覆空間 (covering space)} という．
\end{dfn}

\begin{ex}
  $S^2$ の被覆空間は様々あるが，その中の一つに $\mathbb{R}$ がある(図\ref{S2coveringspace}参照)． 
...
\begin{dfn}
  位相空間 $X$ の 被覆空間 $\widetilde{X}$ が \textbf{普遍被覆 (universal cover)} であるとは，$\widetilde{X}$ が弧状連結かつ $\pi_1(\widetilde{X})$ が自明群であることをいう．また，$p$ を省略し，単に $\widetilde{X}$ だけを普遍被覆ともいうこともある．
\end{dfn}

\begin{ex}
  図\ref{S2coveringspace} より，$\mathbb{R}^2$ は $S^2$ の普遍被覆である．
\end{ex}
...
\begin{ex}
  $S^2$ の被覆空間 $\mathbb{R}$ の被覆変換は 図\ref{decktrans} などがある．これは，$p\colon \mathbb{R}\rightarrow S^2\subset \mathbb{R}^2\;;\; t\mapsto (\cos2\pi t,\sin2\pi t)$ としたとき，$f\colon \mathbb{R}\rightarrow \mathbb{R}\; ;\; t\mapsto t+2\pi k\; (k\in\mathbb{Z})$ などが挙げられる．
\begin{figure}[h]
\centering
