...
\begin{lem}\label{lem1}
  Lemma \ref{svarc} の証明において，群作用が $(c^\prime,b^\prime)$-擬等長な作用でも $x_i\in g_{i-1}\cdot B^\prime$ が成立する．ただし，$B^\prime :=\{x\in X\mid y\in X，d_X(x,y)\leq 2bc^\prime+b^\prime\}$ とする．
\end{lem}
\begin{proof}
...
\begin{lem}\label{lem2}
  Lemma \ref{svarc} の証明において，群作用が $(c^\prime,b^\prime)$-擬等長な作用でも $\mathrm{diam}(g^\prime\cdot B)< \infty$ である． 
\end{lem}
\begin{proof}
   群作用が $(c^\prime,b^\prime)$-擬等長なので，
...
$$\tilde{g}_{\tilde{x}}(u,v):=g_{p(\tilde{x})}(dp_{\tilde{x}}(u),dp_{\tilde{x}}(v))$$
...
\begin{dfn}\label{cayley}
  生成系 $S$ による群 $G$ の \textbf{cayleyグラフ (cayley graph)} とは，以下のようなグラフである．このグラフを $\mathrm{Cay}(G,S)$ と表記する．
  \begin{enumerate}
    \item 頂点集合を $G$ とする．
...
\begin{rem}
$g(x)$ を $g\cdot x$ と表記する．また，部分集合 $A\subset X$ に対し，$g\cdot A:=\{g\cdot x\mid x\in A\}$ とする．
\end{rem}
...
\begin{ex}
  群 $\mathbb{Z}^2$ の集合 $\mathbb{R}^2$ への加法での群作用は自由である．しかし，回転群 $\mathrm{SO}(2):=\{A\in \mathrm{GL}_2(\mathbb{R})\mid \mathrm{det}A=1\}$ の $\mathbb{R}^2$ への群作用は自由でない．点 $(0,0)\in \mathbb{R}^2$ は$\mathrm{SO}(2)$ のいかなる元を作用させても不動だからである．
\end{ex}
...
\begin{dfn}\label{homotopy}
  $X$ を位相空間とする．$I:=[0,1]$ として，連続写像 $f\colon I\rightarrow X$ を \textbf{パス (path)} といい，$f(0)=f(1)$ のときを特に \textbf{ループ (loop)} という．また，パスの族 $f_t\colon I\rightarrow X\;(0\leq t\leq 1)$ が \textbf{ホモトピー (homotopy)} であるとは，以下の二条件：
  \begin{enumerate}
    \item $f_t(0)=x_0$，$f_t(1)=x_1$ が $t\in I$ に依らない．
    \item $F(s,t):=f_t(s)$ が，$s,t$ に関して連続．
  \end{enumerate}
...
\begin{rem}
  $X$ が弧状連結のとき，$X$ による基本群は基点に依らない．そこで，$X$ が弧状連結のときは，基点を省略して単に $\pi_1(X)$ と表記する．
\end{rem}
...
\begin{center}
\begin{tikzpicture}
\draw[samples=100,domain=0:2*pi,thick] plot({2*cos(\x r)},{sin(\x r)});
\draw[samples=300,domain=0:6*pi,thick] plot({2*cos(\x r)},{sin(\x r)+\x/4+1.5});
\draw (-1,-1.9/2)node[below]{$x$};
\draw (2,0)node[right]{$X$};
\draw (2,3)node[right]{$\tilde{X}$};
\draw[->,samples=100,domain=2/3*pi:4/3*pi] plot({cos(\x r)-2},{2*sin(\x r)+2});
\draw (-3,2)node[left]{$p$};
\draw(-1.4,-1.4/2) circle (0.1); %5/4pi
\draw(0,-1) circle (0.1); %3/2pi
\draw[samples=100,domain=5/4*pi:3/2*pi,thick] plot({2*cos(\x r)},{sin(\x r)});
\draw(-0.8,-1.7/2)node[above]{$U$};
\draw(-1.4,-1.4/2+1.5+0.98) circle (0.1); 
\draw(-1.4,-1.4/2+1.5+2.55) circle (0.1); 
\draw(-1.4,-1.4/2+1.5+4.12) circle (0.1); 
\draw(0,-1+1.5+1.18) circle (0.1); 
\draw(0,-1+1.5+2.75) circle (0.1); 
\draw(0,-1+1.5+4.32) circle (0.1); 
\draw(-0.8,1.6)node[above]{$\tilde{U}_1$};
\draw(-0.8,1.6+1.57)node[above]{$\tilde{U}_2$};
\draw(-0.8,1.6+1.57*2)node[above]{$\tilde{U}_3$};
\draw[samples=100,domain=5/4*pi:3/2*pi,blue,thick] plot({2*cos(\x r)},{sin(\x r)+\x/4+1.5});
\draw[samples=100,domain=(5/4+2)*pi:(3/2+2)*pi,blue,thick] plot({2*cos(\x r)},{sin(\x r)+\x/4+1.5});
\draw[samples=100,domain=(5/4+4)*pi:(3/2+4)*pi,blue,thick] plot({2*cos(\x r)},{sin(\x r)+\x/4+1.5});
\fill[black] (-1,-1.7/2) circle (0.1); %4/3pi
\end{tikzpicture}
\end{center}
\end{ex}
...
\begin{dfn}
  ...
  \begin{equation*}
    L(\gamma):=\int_{a}^{b}\Bigl\|\frac{d\gamma}{dt}\Bigr\|dt
  \end{equation*}
  を曲線 $\gamma$ の \textbf{長さ (length)} といい，二点間 $x_0,x_1\in M$ の距離 $d_M$ を，
  \begin{equation*}
    d_M(x_0,x_1):=\mathrm{inf}\{L(\gamma)\mid \gamma(a)=x_0,\gamma(b)=x_1\}
  \end{equation*}
  と定める．
\end{dfn}
