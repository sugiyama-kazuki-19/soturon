...
$$ (dp)_{\tilde{x}}\colon T_{\tilde{x}}(\widetilde{M})\rightarrow T_{p(\tilde{x})}(M) $$
とし，$\widetilde{M}$ 上の点 $\tilde{x}$ におけるリーマン計量 $\tilde{g}_{\tilde{x}}$ を，
$$\tilde{g}_{\tilde{x}}(u,v):=g_{p(\tilde{x})}((dp)_{\tilde{x}}(u),(dp)_{\tilde{x}}(v))$$
とすることで，$\widetilde{M}$ に距離が入る．\\

\noindent
群作用が等長であること：\\
%$\widetilde{M}$ には，$M$ から誘導される距離が入っているので，被覆変換の定義より等長である．
...
\begin{dfn}
  $M$ を位相多様体とする．各点 $p\in M$ における接空間 $T_p(M)$ の内積 $g_p$ があり，それが $M$ 全体で $C^\infty$ 級のとき，$(M,\{g_p\}p\in M)$ を \textbf{リーマン多様体} という．
\end{dfn}
